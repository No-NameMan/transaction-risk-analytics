...
def score_merchants() -> pd.DataFrame:
    ...
    merchants["flag_high_turnover_vs_peer"] = (merchants["turnover_peer_ratio"] >= 3.0).astype(int)
    merchants["flag_high_avg_ticket_vs_peer"] = (merchants["avg_ticket_peer_ratio"] >= 2.0).astype(int)
    merchants["flag_high_tx_count_vs_peer"] = (merchants["tx_count_peer_ratio"] >= 3.0).astype(int)
    merchants["flag_high_night_share"] = (merchants["night_tx_share"] >= 0.30).astype(int)
    merchants["flag_high_declined_share"] = (merchants["declined_share"] >= 0.08).astype(int)

    merchants["risk_score"] = 100 * (
        0.30 * clipped_excess_ratio(merchants["turnover_peer_ratio"], baseline=1.0, upper_excess=4.0)
        + 0.20 * clipped_excess_ratio(merchants["avg_ticket_peer_ratio"], baseline=1.0, upper_excess=3.0)
        + 0.20 * clipped_excess_ratio(merchants["tx_count_peer_ratio"], baseline=1.0, upper_excess=4.0)
        + 0.20 * clipped_share(merchants["night_tx_share"], upper=0.50)
        + 0.10 * clipped_share(merchants["declined_share"], upper=0.15)
    )

    merchants["risk_score"] = merchants["risk_score"].round(1)
    merchants["risk_level"] = assign_risk_level(merchants["risk_score"])
...
def score_users() -> pd.DataFrame:
    ...
    users["risk_level"] = assign_risk_level(users["risk_score"])
